omni_search(start=""){
	...
	while,bb:=bm.item[A_Index-1],ea:=bookmarks.ea(bb)
		if !cexml.ssn("//bookmark/*[@file='" ssn(bb.ParentNode,"@file").text "']")
			cexml.under({under:top,node:"bookmark",att:{type:"bookmark",text:ea.name,line:ea.line,file:ssn(bb.ParentNode,"@file").text,order:"text,type,file",root:ssn(bb,"ancestor::main/@file").text}})
	oss:
	break:=1
	...
	if (type:=item.launch){
		...
		Sleep,200
		csc().2024(item.line),hwnd({rem:20})
