Add_Button(this=""){
	...
	if (A_ThisLabel="abrun"){
		...
		iconfile:=InStr(filename,".ahk")?A_AhkPath:filename,att:={icon:0,file:iconfile,text:nne,func:"runfile",id:newid,state:4,runfile:filename},bar.add(att),bar.addbutton(newid),settings.under({under:under,node:"button",att:att})
	}else
		LV_GetText(item,LV_GetNext()),text:=RegExReplace(item,"_"," "),att:={icon:1,file:"shell32.dll",text:text,func:item,id:newid,state:4},bar.add(att),bar.addbutton(newid),settings.under({under:under,node:"button",att:att})
	return
	12GuiClose:
	12GuiEscape:
	hwnd({rem:12})
